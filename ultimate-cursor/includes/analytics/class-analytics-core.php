...
if (!class_exists('WPXERO_Analytics')) {
	class WPXERO_Analytics {
		private $version;
		private $product_id;
		private $plugin_name;
		...
		// Static property to track if any plugin has shown a notice
		...
		public function __construct($params) {
			...
			$this->analytics_key = 'wpxero_' . md5($this->plugin_name);
		...
		private function init_hooks() {
			...
			add_action('wp_ajax_wpxero_analytics_data', array($this, 'handle_analytics_request'));
			add_action('wp_ajax_wpxero_dismiss_notice', array($this, 'handle_notice_dismissal'));

			if ($this->deactivate_feedback) {
				add_action('wp_ajax_wpxero_deactivate_feedback', array($this, 'handle_deactivate_feedback'));
		...
		private function maybe_show_notice() {
			// Check if another plugin has already displayed a notice
			if (self::$notice_displayed) {
				return;
			}

			// Check if this notice is already dismissed
			$notice_status = get_option('wpxero_notice_status_' . $this->analytics_key, false);
			if (!$notice_status) {
				// Mark that a notice will be displayed
		...
		public function handle_analytics_request() {
			...
			switch ($action) {
				case 'enable':
					update_option('wpxero_analytics_status_' . $this->analytics_key, 'enabled');
					$this->send_initial_analytics();
					break;
				case 'disable':
					update_option('wpxero_analytics_status_' . $this->analytics_key, 'disabled');
					break;
			}
		...
		public function handle_notice_dismissal() {
			...
			update_option('wpxero_notice_status_' . $this->analytics_key, 'dismissed');

			// Reset the static notice flag when a notice is dismissed
		...
		private function maybe_send_analytics() {
			...
			$analytics_status = get_option('wpxero_analytics_status_' . $this->analytics_key, false);
